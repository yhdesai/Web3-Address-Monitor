...
var subscription1 = web3.eth
  ...
  .on("data", function (log) {
    //console.log(log);
    web3.eth.getTransaction(log.transactionHash).then(function (transaction) {
      //console.log(transaction)
      if (transaction.value > 0) {
        console.log(
          "Ether sent from " + transaction.from + " to " + transaction.to
        );
        newTransactionLog.send({
          text: "Ether sent from " + transaction.from + " to " + transaction.to,
        });
      }
    });
  });
